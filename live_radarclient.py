...
def readAndParseData2(Dataport, configParameters, sensor_id):
    ...
    if (byteBufferLength2 + byteCount) < maxBufferSize:
        byteBuffer2[byteBufferLength2:byteBufferLength2 + byteCount] = byteVec[:byteCount]
    ...
    if byteBufferLength2 > 16:
        ...
        if startIdx:
            ...
            if startIdx[0] > 0 and startIdx[0] < byteBufferLength2:
                ...
                byteBuffer2Length = byteBufferLength2 - startIdx[0]
    ...
    if magicOK:
        ...
        for tlvIdx in range(numTLVs):
            ...
            # Check the header of the TLV message
            #print(f"byteBuffer2[idX:idX+4] = {byteBuffer2[idX:idX+4]}, word = {word}")
            tlv_type, tlv_length= tlvHeaderDecode(byteBuffer2[idX:idX+8])
            
            #tlv_type = np.matmul(byteBuffer2[idX:idX+4],word)
            idX += 4
            print('tlv_type is: '+str(tlv_type))
            #tlv_length = np.matmul(byteBuffer2[idX:idX+4],word)
            idX += 4
            print('tlv_length is: '+str(tlv_length))
            #print(f"tlv_type = {tlv_type} \t MMWDEMO_UART_MSG_DETECTED_POINTS = {MMWDEMO_UART_MSG_DETECTED_POINTS}")
            # Read the data depending on the TLV message
        ...
        if idX > 0 and byteBufferLength2 > idX:
            ...
            byteBuffer2[:byteBufferLength2 - shiftSize] = byteBuffer2[shiftSize:byteBufferLength2]
            byteBuffer2[byteBufferLength2 - shiftSize:] = np.zeros(len(byteBuffer2[byteBufferLength2 - shiftSize:]),dtype = 'uint8')
...
def update(client):
    ...
    dataOk1, frameNumber, detObj1 = readAndParseData14xx(Dataport1, configParameters, sensor_id=1)
    dataOk2, frameNumber, detObj2 = readAndParseData14xx2(Dataport2, configParameters, sensor_id=2)
    #print(f"dataOK = {dataOk}")
    #if dataOk and len(detObj["x"]) > 0:
        #print(detObj)
        #update_demo(detObj)
    
    return dataOk1, dataOk2
...
CLIport1, Dataport1 = serialConfig1(configFileName, CLICOM1, DataCOM1)
time.sleep(0.1)
CLIport2, Dataport2 = serialConfig2(configFileName, CLICOM2, DataCOM2)
